...
class PriceLadder {
  public:
    ...
    bool has_sufficient_marketable_liquidity(Price thresholdPrice, Quantity neededLiquidity) const;
...
void PriceLadder::prune_day_orders(DayOrderPruneResult& dayResult, OnPrunedOrder&& onPrunedOrder) {
    for (std::size_t i = 0; i < levels_.size(); ++i) {
        if (!levels_[i].empty()) {
            ...
            const core::LevelPruneStats levelStats =
                level.prune_day_orders([&](core::RestingOrder* order) { onPrunedOrder(order); });
