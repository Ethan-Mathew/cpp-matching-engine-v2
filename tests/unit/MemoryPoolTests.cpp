...
class MemoryPoolTest : public testing::Test {
  protected:
    MemoryPoolTest() : mp_{initialSlabSize} {}

    static RestingOrder* allocate_order(MemoryPool& mp, lob::OrderID id, lob::Quantity qty,
...
TEST_F(MemoryPoolTest, MultipleAllocationsConstructDistinctLiveObjects) {
    ...
    for (std::uint64_t i = 1; i <= initialSlabSize; ++i) {
        orders.push_back(allocate_order(mp_, static_cast<lob::OrderID>(i),
                                        static_cast<lob::Quantity>(i), RestingLifetime::GTC,
                                        lob::Side::BUY));
    }
...
TEST_F(MemoryPoolTest, FreedBlocksAreReusedWithoutGrowingPool) {
    ...
    for (std::uint64_t i = 1; i <= initialSlabSize; ++i) {
        firstRound.push_back(allocate_order(mp_, static_cast<lob::OrderID>(i),
                                            static_cast<lob::Quantity>(i), RestingLifetime::GTC,
                                            lob::Side::BUY));
    }
    ...
    for (std::uint64_t i = 1; i <= initialSlabSize; ++i) {
        secondRound.push_back(allocate_order(mp_, static_cast<lob::OrderID>(i),
                                             static_cast<lob::Quantity>(i), RestingLifetime::GTC,
                                             lob::Side::SELL));
    }
...
TEST_F(MemoryPoolTest, PoolGrowsByNewSlabWhenCapacityIsExceeded) {
    ...
    orders.push_back(allocate_order(mp_, static_cast<lob::OrderID>(initialSlabSize + 1),
                                    static_cast<lob::Quantity>(initialSlabSize + 1),
                                    RestingLifetime::GTC, lob::Side::BUY));
    ...
    orders.push_back(allocate_order(mp_, static_cast<lob::OrderID>(initialSlabSize * 2 + 1),
...
TEST_F(MemoryPoolTest, ReusesFreedBlocksBeforeGrowingPool) {
    ...
    for (std::uint64_t i = 1; i <= initialSlabSize; ++i) {
        initialOrders.push_back(allocate_order(mp_, static_cast<lob::OrderID>(i),
                                               static_cast<lob::Quantity>(i), RestingLifetime::GTC,
                                               lob::Side::BUY));
    }
    ...
    for (std::uint64_t i = 1; i <= 10; ++i) {
        reusedOrders.push_back(allocate_order(mp_, static_cast<lob::OrderID>(1000 + i),
                                              static_cast<lob::Quantity>(2000 + i),
                                              RestingLifetime::DAY, lob::Side::SELL));
    }
