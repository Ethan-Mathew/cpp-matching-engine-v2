...
// Cancel same-level bid orders in FIFO order, repeatedly removing the current head
...
// Cancel same-level bid orders in reverse FIFO order, repeatedly removing the current tail
...
// Cancel middle orders from independent same-level triples, exercising middle-node unlinking
...
// Cancel orders that each occupy a unique bid price level, forcing level erasure
