...
static void BM_SubmitIOCNoFill(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        for (std::size_t i = 0; i < batchSize; ++i) {
            LimitOrderRequest request{i + 1, 1, 1, Side::BUY, TimeInForce::IOC};
...
static void BM_SubmitIOCSameLevelPartialFill(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        LimitOrderRequest restingAsk{1, 1, static_cast<Quantity>(batchSize + 1), Side::SELL,
                                     TimeInForce::GTC};
        ...
        for (std::size_t i = 0; i < batchSize; ++i) {
            LimitOrderRequest request{i + 2, 1, 1, Side::BUY, TimeInForce::IOC};
...
static void BM_SubmitIOCSameLevelFullFill(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        for (std::size_t i = batchSize + 1; i <= doubleBatchSize; ++i) {
            LimitOrderRequest request{i, 1, 1, Side::BUY, TimeInForce::IOC};
...
template <std::size_t SweepDepth> static void BM_SubmitIOCSweepLevels(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        for (std::size_t i = 0; i < batchSize; ++i) {
            ...
            LimitOrderRequest aggressiveBuy{restingOrderCount + i + 1, buyPrice,
