...
template <std::size_t ScanDepth>
static void BM_SubmitFOKKilledAfterScan(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        for (std::size_t i = 0; i < batchSize; ++i)
        {
            LimitOrderRequest request{
                ...
                static_cast<Quantity>(ScanDepth + 1),
                Side::BUY,
                TimeInForce::FOK
            };
...
static void BM_SubmitFOKSweepLevels(benchmark::State& state)
{
