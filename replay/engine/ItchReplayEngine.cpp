...
ItchReplayEngine::ItchReplayEngine(const std::string& symbol, const OrderBookConfig& obConfig)
    : symbol_{symbol},
      targetStockLocate_{std::nullopt},
      ob_{obConfig}
{
}
...
void ItchReplayEngine::on_message(const data::AddOrderMessage& message)
{
    ...
    const bool applied = ob_.replay_add_visible_order(
                             message.orderReferenceNumber_,
                             message.price_,
                             message.shares_,
...
void ItchReplayEngine::on_message(const data::OrderExecutedMessage& message)
{
    ...
    const bool applied = ob_.replay_reduce_visible_order(message.orderReferenceNumber_, message.executedShares_);
...
void ItchReplayEngine::on_message(const data::OrderCancelMessage& message)
{
    ...
    const bool applied = ob_.replay_reduce_visible_order(message.orderReferenceNumber_, message.cancelledShares_);
...
void ItchReplayEngine::on_message(const data::OrderReplaceMessage& message)
{
    ...
    const bool applied = ob_.replay_replace_visible_order(
                             message.originalOrderReferenceNumber_,
...
void ItchReplayEngine::print_summary(std::ostream& out, std::size_t depth) const
{
    ...
    {
        out << "Best Bid: "
            << static_cast<double>(*bestBid) / 10'000
            << " x "
    ...
    {
        out << "Best Ask: "
            << static_cast<double>(*bestAsk) / 10'000
            << " x "
...
bool ItchReplayEngine::matches_symbol(const std::array<char, 8>& itchStock, const std::string& symbol)
{
